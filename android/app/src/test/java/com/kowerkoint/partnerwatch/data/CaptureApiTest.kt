...
class CaptureApiTest {
    @Test fun parsesCreatedRequest() {
        val result = CaptureApi().parseCreated(
            """{"requests":[{"requestId":"request","targetDeviceName":"niri PC","targetPlatform":"LINUX","status":"PENDING","createdAt":"2026-08-31T00:00:00Z","expiresAt":"2026-08-31T00:01:00Z"}]}""",
        ).single()
        assertEquals("request", result.requestId)
        assertEquals("niri PC", result.targetDeviceName)
